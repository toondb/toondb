...
//! | [`ToonConnection`] | In-memory only | Testing - ephemeral data |
//!
//! **For production workloads, always use [`DurableConnection`]**:
//!
//! ```rust,ignore
//! use toondb::DurableConnection;
//!
...
use toondb_core::catalog::Catalog;
use toondb_core::toon::{ToonSchema, ToonType, ToonValue};
...
/// ## Migration
/// 
/// Replace with `toondb_storage::WalStorageManager` or use [`DurableConnection`]:
/// 
/// ```rust,ignore
...
/// ## Migration
///
/// Replace with `toondb_storage::MvccTransactionManager` or use [`DurableConnection`]:
///
/// ```rust,ignore
...
impl From<&ToonValue> for ColumnValue {
    fn from(v: &ToonValue) -> Self {
        match v {
            ToonValue::Null => ColumnValue::Null,
            ToonValue::Int(i) => ColumnValue::Int64(*i),
            ToonValue::UInt(u) => ColumnValue::UInt64(*u),
            ToonValue::Float(f) => ColumnValue::Float64(*f),
            ToonValue::Text(s) => ColumnValue::Text(s.clone()),
            ToonValue::Bool(b) => ColumnValue::Bool(*b),
            ToonValue::Binary(b) => ColumnValue::Bytes(b.clone()),
            _ => ColumnValue::Text(format!("{:?}", v)), // Fallback for complex types
        }
    }
}

impl From<ColumnValue> for ToonValue {
    fn from(val: ColumnValue) -> Self {
        match val {
            ColumnValue::Null => ToonValue::Null,
            ColumnValue::UInt64(u) => ToonValue::UInt(u),
            ColumnValue::Int64(i) => ToonValue::Int(i),
            ColumnValue::Float64(f) => ToonValue::Float(f),
            ColumnValue::Text(s) => ToonValue::Text(s),
            ColumnValue::Bool(b) => ToonValue::Bool(b),
            ColumnValue::Bytes(b) => ToonValue::Binary(b),
        }
    }
}
...
impl ColumnStore {
    ...
    fn insert(
        ...
        table: &str,
        row_id: u64,
        values: &std::collections::HashMap<String, ToonValue>,
    ) -> bool {
    ...
    fn get_row(
        ...
        row_idx: usize,
        columns: &[String],
    ) -> Option<std::collections::HashMap<String, ToonValue>> {
    ...
    fn get_all_rows(
        ...
        table: &str,
        columns: &[String],
    ) -> Vec<(usize, std::collections::HashMap<String, ToonValue>)> {
    ...
    fn iter_rows_batched<'a>(
        ...
        columns: &'a [String],
        batch_size: usize,
    ) -> impl Iterator<Item = Vec<(usize, std::collections::HashMap<String, ToonValue>)>> + 'a {
        let table_cols = self.columns.get(table);
        let row_meta = self.row_meta.get(table);
    ...
    fn update_row(
        ...
        table: &str,
        row_idx: usize,
        updates: &std::collections::HashMap<String, ToonValue>,
    ) -> bool {
...
impl TrieColumnarHybrid {
    ...
    /// O(|path|) path resolution - ToonDB's key differentiator
    ...
    fn matches_where(
        &self,
        row: &std::collections::HashMap<String, ToonValue>,
        wc: &WhereClause,
    ) -> bool {
    ...
    /// Compare two ToonValues (legacy - kept for backward compatibility)
    fn compare_values(&self, a: &ToonValue, b: &ToonValue) -> i32 {
        match (a, b) {
            (ToonValue::Int(a), ToonValue::Int(b)) => a.cmp(b) as i32,
            (ToonValue::UInt(a), ToonValue::UInt(b)) => a.cmp(b) as i32,
            (ToonValue::Float(a), ToonValue::Float(b)) => {
                if a < b {
                    -1
                } else if a > b {
                    1
            ...
            (ToonValue::Text(a), ToonValue::Text(b)) => a.cmp(b) as i32,
            _ => 0,
        }
    }
    ...
    pub fn select(
        ...
        let mut filtered: Vec<std::collections::HashMap<String, ToonValue>> = Vec::with_capacity(estimated_size);
...
impl WhereClause {
    ...
    pub fn eq(field: impl Into<String>, value: ToonValue) -> Self {
        WhereClause::Simple {
            field: field.into(),
            op: CompareOp::Eq,
            value,
    ...
    pub fn compare(field: impl Into<String>, op: CompareOp, value: ToonValue) -> Self {
        WhereClause::Simple {
            field: field.into(),
            op,
            value,
    ...
    /// Create an IN clause
    pub fn in_values(field: impl Into<String>, values: Vec<ToonValue>) -> Self {
        WhereClause::In {
            field: field.into(),
            values,
            negated: false,
    ...
    pub fn not_in(field: impl Into<String>, values: Vec<ToonValue>) -> Self {
        WhereClause::In {
            field: field.into(),
            values,
            negated: true,
    ...
    pub fn matches(&self, row: &std::collections::HashMap<String, ToonValue>) -> bool {
        match self {
            ...
            WhereClause::IsNull { field, negated } => {
                let is_null = row.get(field).map(|v| matches!(v, ToonValue::Null)).unwrap_or(true);
...
fn compare_values(left: &ToonValue, op: &CompareOp, right: &ToonValue) -> bool {
    match (left, right) {
        (ToonValue::Int(l), ToonValue::Int(r)) => match op {
            CompareOp::Eq => l == r,
            CompareOp::Ne => l != r,
            CompareOp::Lt => l < r,
            CompareOp::Le => l <= r,
        ...
        (ToonValue::UInt(l), ToonValue::UInt(r)) => match op {
            CompareOp::Eq => l == r,
            CompareOp::Ne => l != r,
            CompareOp::Lt => l < r,
            CompareOp::Le => l <= r,
        ...
        (ToonValue::Float(l), ToonValue::Float(r)) => match op {
        ...
        (ToonValue::Text(l), ToonValue::Text(r)) => match op {
            CompareOp::Eq => l == r,
            CompareOp::Ne => l != r,
            CompareOp::Lt => l < r,
            CompareOp::Le => l <= r,
        ...
        (ToonValue::Bool(l), ToonValue::Bool(r)) => match op {
            CompareOp::Eq => l == r,
            CompareOp::Ne => l != r,
            _ => false,
        },
        (ToonValue::Null, ToonValue::Null) => matches!(op, CompareOp::Eq),
...
pub struct ToonCursor {
    rows: Vec<std::collections::HashMap<String, ToonValue>>,
    position: usize,
}

impl Default for ToonCursor {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ToonCursor {
    ...
    pub fn next(&mut self) -> Option<std::collections::HashMap<String, ToonValue>> {
...
/// ToonDB In-Memory Connection (Testing Only)
///
...
impl ToonConnection {
    ...
    pub fn open(_path: impl AsRef<Path>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            catalog: Arc::new(RwLock::new(Catalog::new("toondb"))),
            active_txn: RwLock::new(None),
            queries_executed: AtomicU64::new(0),
            toon_tokens_emitted: AtomicU64::new(0),
            json_tokens_equivalent: AtomicU64::new(0),
        })
    }

    /// Path-based access - ToonDB's differentiator
    ...
    pub fn get_schema(&self, table: &str) -> Result<ToonSchema> {
    ...
    #[allow(dead_code)]
    pub(crate) fn record_tokens(&self, toon_tokens: usize, json_tokens: usize) {
        self.toon_tokens_emitted
            .fetch_add(toon_tokens as u64, Ordering::Relaxed);
    ...
    pub fn stats(&self) -> ClientStats {
        let toon = self.toon_tokens_emitted.load(Ordering::Relaxed);
    ...
    /// Serialize a ToonValue to bytes
    pub fn serialize_value(&self, value: &ToonValue) -> Result<Vec<u8>> {
        bincode::serialize(value).map_err(|e| ClientError::Serialization(e.to_string()))
    }

    /// Deserialize bytes to ToonValue
    pub fn deserialize_value(&self, bytes: &[u8]) -> Result<ToonValue> {
...
/// Convert ToonType to FieldType
pub fn to_field_type(toon_type: &ToonType) -> FieldType {
    match toon_type {
        ToonType::Int => FieldType::Int64,
        ToonType::Float => FieldType::Float64,
        ToonType::Text => FieldType::Text,
        ToonType::Bool => FieldType::Bool,
        ToonType::Binary => FieldType::Bytes,
        ToonType::UInt => FieldType::Int64,
...
pub use toondb_storage::database::{
...
use toondb_storage::database::{Database, DatabaseConfig, TxnHandle as KernelTxnHandle};
...
impl EmbeddedConnection {
    ...
    // =========================================================================
    // Path API (ToonDB's differentiator)
    // =========================================================================
    ...
    pub fn read_row(
        ...
        row_id: u64,
        columns: Option<&[&str]>,
    ) -> Result<Option<std::collections::HashMap<String, ToonValue>>> {
        let txn = self.ensure_txn()?;
        self.db
...
///
/// Mirrors toondb_storage::DatabaseConfig but exposed at the client level.
#[derive(Debug, Clone)]
...
impl DurableConnection {
    ...
    pub fn insert_row_slice(
        ...
        table: &str,
        row_id: u64,
        values: &[Option<&toondb_core::toon::ToonValue>],
    ) -> Result<()> {
        let txn_id = self.ensure_txn()?;
    ...
    where
        I: IntoIterator<Item = (u64, Vec<Option<&'a toondb_core::toon::ToonValue>>)>,
    {
        let mut count = 0;
        ...
        for (row_id, values) in rows {
            ...
            let value_refs: Vec<Option<&toondb_core::toon::ToonValue>> = values;
...
mod tests {
    ...
    fn test_connection_open() {
        let conn = ToonConnection::open("./test_data").unwrap();
    ...
    fn test_stats() {
        ...
        assert_eq!(stats.toon_tokens_emitted, 100);
    ...
    fn test_tch_insert_and_select() {
        ...
        row1.insert("id".to_string(), ToonValue::UInt(1));
        row1.insert("name".to_string(), ToonValue::Text("Alice".to_string()));
        row1.insert("score".to_string(), ToonValue::Float(95.5));
        let id1 = tch.insert_row("users", &row1);
        assert_eq!(id1, 1);

        let mut row2 = std::collections::HashMap::new();
        row2.insert("id".to_string(), ToonValue::UInt(2));
        row2.insert("name".to_string(), ToonValue::Text("Bob".to_string()));
        row2.insert("score".to_string(), ToonValue::Float(87.2));
    ...
    fn test_tch_where_clause() {
        ...
        for i in 1..=5 {
            let mut row = std::collections::HashMap::new();
            row.insert("id".to_string(), ToonValue::UInt(i));
            row.insert("name".to_string(), ToonValue::Text(format!("User{}", i)));
            row.insert("score".to_string(), ToonValue::Float((i * 20) as f64));
    ...
    fn test_tch_update_and_delete() {
        ...
        let mut row = std::collections::HashMap::new();
        row.insert("id".to_string(), ToonValue::UInt(1));
        row.insert("name".to_string(), ToonValue::Text("Alice".to_string()));
        tch.insert_row("users", &row);
        ...
        updates.insert(
            "name".to_string(),
            ToonValue::Text("Alice Updated".to_string()),
        );
        let where_clause = WhereClause::Simple {
            field: "id".to_string(),
            op: CompareOp::Eq,
            value: ToonValue::UInt(1),
        };
        ...
        assert_eq!(
            rows[0].get("name"),
            Some(&ToonValue::Text("Alice Updated".to_string()))
        );
    ...
    fn test_tch_upsert() {
        ...
        let mut row2 = std::collections::HashMap::new();
        row2.insert("id".to_string(), ToonValue::UInt(1));
        row2.insert(
            "name".to_string(),
            ToonValue::Text("Alice Updated".to_string()),
        );
